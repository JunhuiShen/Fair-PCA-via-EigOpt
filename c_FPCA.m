function proj_matrix = c_FPCA(A, B, r, tol)
    ...
    proj_matrix = golden_section_function_constr(alpha0,alpha1,covM,dif_cov,M,A,B,m,n,na,nb,r,max(recA_pca,recB_pca));

    function proj_matrix = golden_section_function_constr(alpha0,alpha1,covM,dif_cov,M,A,B,m,n,na,nb,r,maxRec)
        ...
        while abs(alpha1-alpha0) > tol
            ...
            if (rec_difs_0 < rec_difs_1)
                if (recA_0 <= maxRec + 10^(-6)) && (recB_0 <= maxRec + 10^(-6))
                    alpha1 = alpha1_aux;
                else
                    alpha0 = alpha0_aux;
                end
