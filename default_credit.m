function [M, A, B] = default_credit()
    ...
    %
    % Output:
    %   M - Combined dataset of low and higher education level clients
    %   A - Data for low education level clients 
    %   B - Data for higher education level clients 

    % Load the dataset from the CSV file
    data = readtable('default_degree.csv');
    data = data{:,:}; % Convert table to matrix
    data = data(:,2:end); % Remove the first column (IDs or unnecessary info)
    data = sortrows(data); % Sort rows for consistent processing
